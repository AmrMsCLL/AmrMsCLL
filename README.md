<div align="center">

<a href="https://www.linkedin.com/in/amrsherieycll"><img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f172a,50:1e3a8a,100:0ea5e9&height=190&section=header&text=Amr%20El-Sheraey&fontSize=52&fontColor=ffffff&fontAlignY=36&desc=Full-Stack%20Engineer%20·%20Cairo,%20Egypt&descAlignY=57&descSize=16&animation=fadeIn" width="100%" alt="Amr El-Sheraey — Full-Stack Engineer, Cairo, Egypt" /></a>

<a href="https://github.com/AmrMsCLL?tab=repositories"><img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=20&duration=3200&pause=900&color=0EA5E9&center=true&vCenter=true&width=560&lines=NestJS+%2B+Prisma+on+the+back;React+%2B+Redux+on+the+front;PyTorch+%2B+Qdrant+in+the+middle;Terraform+%2B+Helm+underneath" alt="What I build with" /></a>

<br/>

<a href="https://github.com/AmrMsCLL?tab=followers"><img src="https://img.shields.io/github/followers/AmrMsCLL?style=for-the-badge&logo=github&logoColor=white&labelColor=0f172a&color=0ea5e9" alt="Followers" /></a>

</div>

---

### About

Computer Science & AI graduate from **FCAI, Cairo University**, based in Cairo. I build
document-intelligence software at **Syntheia**.

I work across the whole stack, which in practice means a normal week touches a TypeScript API, a
React front end, a Python ML service and a Helm chart. I like the layer where those meet.

```
  backend    NestJS · Prisma · PostgreSQL · Redis · GraphQL · SAML/OAuth
  frontend   React · Redux Toolkit · MUI · Tailwind · Vite · Playwright
  ml         PyTorch · Transformers · spaCy · scikit-learn · Celery
  vectors    Qdrant · Milvus
  platform   Docker · Kubernetes · Helm · Terraform · AWS · Azure · SonarQube
  office     Office.js add-ins for Word & Outlook
```

---

### Selected public work

<table>
  <tr>
    <td width="50%" valign="top">
      <h4><a href="https://github.com/AmrMsCLL/Machine-Language-Simulator">Machine Language Simulator</a></h4>
      <p><a href="https://github.com/AmrMsCLL/Machine-Language-Simulator"><img src="https://img.shields.io/badge/C%2B%2B-00599C?style=flat-square&logo=cplusplus&logoColor=white" alt="Written in C++" /></a></p>
      <p>The Vole machine emulated from the metal up — 256 bytes of memory, 16 registers and a
      program counter as separate classes, running a hex instruction set through a menu-driven
      debugger.</p>
    </td>
    <td width="50%" valign="top">
      <h4><a href="https://github.com/AmrMsCLL/Data-Compression">Data Compression</a></h4>
      <p><a href="https://github.com/AmrMsCLL/Data-Compression"><img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" alt="Written in Python" /></a></p>
      <p>Five codecs written from scratch, no libraries: Huffman, Arithmetic, LZW, LZ77, and
      Vector Quantization with LBG codebook training for images.</p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h4><a href="https://github.com/AmrMsCLL/OOP-Gaming-Assignment">OOP Board Games</a></h4>
      <p><a href="https://github.com/AmrMsCLL/OOP-Gaming-Assignment"><img src="https://img.shields.io/badge/C%2B%2B-00599C?style=flat-square&logo=cplusplus&logoColor=white" alt="Written in C++" /></a></p>
      <p>Three games behind one abstract <code>Board</code> interface. Mine was Four in a Row,
      including its minimax AI with alpha-beta pruning and positional scoring.</p>
    </td>
    <td width="50%" valign="top">
      <h4><a href="https://github.com/AmrMsCLL/Online-Library-Project">Online Library</a></h4>
      <p><a href="https://github.com/AmrMsCLL/Online-Library-Project"><img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" alt="Written in JavaScript" /></a></p>
      <p>A nine-page storefront over an 88-book catalogue — search, categories, accounts and
      cataloguing, on vanilla ES modules and <code>localStorage</code>. No framework.</p>
    </td>
  </tr>
</table>

> Most of what I ship day to day lives in private repositories, so the public numbers below — and
> the language split in particular — run a long way behind the real ones.

---

### Stats

<div align="center">

<a href="https://github.com/AmrMsCLL?tab=repositories"><img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=AmrMsCLL&theme=github_dark" width="98%" alt="Contribution summary" /></a>

<a href="https://github.com/AmrMsCLL?tab=repositories"><img height="200" src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=AmrMsCLL&theme=github_dark" alt="Most-committed languages" /></a>

</div>

---

### Reach me

<div align="center">

<a href="mailto:amrsherieycll@gmail.com"><img src="https://img.shields.io/badge/Email-0ea5e9?style=for-the-badge&logo=gmail&logoColor=white&labelColor=0f172a" alt="Email" /></a>
<a href="https://www.linkedin.com/in/amrsherieycll"><img src="https://img.shields.io/badge/LinkedIn-0ea5e9?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=0f172a" alt="LinkedIn" /></a>

</div>
